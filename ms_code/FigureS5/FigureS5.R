...
# heavy chain junction amino acid sequences, assigns clone group (BEAM_Only, Dual_Only, Shared),
...
make_colored_tree_by_edit_distance <- function(clone_data, label, outdir = ".", cache_prefix = "tree_cache", force_rebuild = FALSE) {
  ...
  legend("topright", legend = names(group_colors), col = group_colors, pch = 19, cex = 1.5)
  dev.off()
  ...
  plot(tree, show.tip.label = FALSE, main = paste("Colored CDR3 Tree -", label), cex = 1.5)
  tiplabels(pch = 19, col = tip_colors, cex = 1.75)
  legend("topright", legend = names(group_colors), col = group_colors, pch = 19, cex = 3)
  dev.off()
...
process_upset_merge_tree <- function(matrix_csv, label_tag, test_mode = FALSE) {
  ...
  make_colored_tree_by_edit_distance(merged, paste0(label_tag, ""), outdir = out_dir, force_rebuild = FALSE)
}
